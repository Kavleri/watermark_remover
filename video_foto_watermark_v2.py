...
def get_user_mask(frame_to_mask):
    ...
    window_name = "Tandai Area (s: Proses, r: Reset, q: Batal)"
    cv2.namedWindow(window_name)
    cv2.setMouseCallback(window_name, draw_mask)
    
    print("\n--- Mode Penandaan Area ---")
    print("Gunakan mouse untuk menggambar area yang ingin dihapus.")
    print("Tekan tombol keyboard:")
    print("  's' - untuk MEMPROSES gambar dengan tanda yang ada.")
    print("  'r' - untuk MENGHAPUS semua tanda (Reset).")
    print("  'q' - untuk MEMBATALKAN dan keluar.")
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            cv2.destroyAllWindows()
            if np.all(mask == 0):
                print("Peringatan: Anda belum menandai area apapun. Proses dibatalkan.")
                return None
            print("Tanda disimpan, melanjutkan ke proses AI...")
            return mask 
            
        elif key == ord('r'):
            print("Tanda telah di-reset.")
            mask = np.zeros(frame_to_mask.shape[:2], dtype="uint8")
